...
DECLARE
    ...
    split            int;
BEGIN
    CREATE TEMP TABLE IF NOT EXISTS staging.partitions
    (
    ...
    INSERT INTO partitions (geom, numPoints, level) VALUES (gdgeom, maxPoints, 1);


    -- while count of partitions is less than maxNumPartitions
    WHILE (SELECT count(*) FROM partitions) < maxNumPartitions
        LOOP
            -- print the amount of partitions currently.
            RAISE NOTICE 'partitions: %', (SELECT count(*) FROM partitions);
            ...
            DELETE FROM partitions p WHERE p.partition_id = t_partition_id;
    ...
    RETURN QUERY SELECT * FROM partitions;
END;
$$ LANGUAGE plpgsql;