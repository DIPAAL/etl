...
SELECT
    ...
    i2.ship_type_id,
    i2.rast,
    (SELECT heatmap_type_id FROM dim_heatmap_type WHERE slug = 'time') AS heatmap_type_id,
    ...
    (
        SELECT
            ST_Union(
                ST_AsRaster(
                    ...
                    '32BUI'::text,
                    -- convert time_sum to seconds from interval
                    EXTRACT(EPOCH FROM i1.time_sum)
                )
            ) AS rast,
        ...
        (
            SELECT
                ...
                fc.infer_stopped,
                fc.partition_id,
                SUM(duration(st_bounding_box::tstzspan)) AS time_sum
