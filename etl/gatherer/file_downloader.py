...
def ensure_file_for_date(date: datetime, config) -> str:
    ...
    # Check if our current date is in the list of available files, if not,
    # check if the month is in the list of available files.
    if not date in file_names:
        ...
        if not date in file_names:
            raise Exception(
                f"File for {date} not found as existing on Danish Maritime Authority website.")
    ...
    # In case the downloaded file did not actually contain the desired date,
    # raise an exception.
    if not os.path.isfile(os.path.join(
            config['DataSource']['ais_path'], expected_filename)):
        raise Exception(
            f"Expected file {expected_filename} was not found in {config['DataSource']['ais_path']}")
...
def date_from_filename(file_name):
    ...
    # If the file name does not contain a day, assume the first day of the
    # month.
...
def ensure_file(file: AisFile, config):
    ...
    if not os.path.isfile(path):
        wrap_with_timings(
            f"Downloading file: {file.name}",
            lambda: wget.download(
                file.url,
                out=path))
    else:
        print(f"File already exists: {path}")
