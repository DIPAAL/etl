...
def _create_trajectory(mmsi: int, data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reset the index as some rows might have been classified as outliers and
    # removed
...
def _construct_moving_trajectory(
        mmsi: int, trajectory_dataframe: gpd.GeoDataFrame, from_idx: int) -> pd.DataFrame:
            ...
            if idx_cannot_handle is not None:
                ...
                if (current_date - trajectory_dataframe.iloc[idx_cannot_handle]
                    ...
                    trajectories = _construct_stopped_trajectory(
                        mmsi, trajectory_dataframe, idx_cannot_handle)
    ...
    return _finalize_trajectory(mmsi, trajectory_dataframe, from_idx, len(
        trajectory_dataframe.index), infer_stopped=False)


def _finalize_trajectory(mmsi: int, trajectory_dataframe: gpd.GeoDataFrame,
                         from_idx: int, to_idx: int, infer_stopped: bool) -> pd.DataFrame:
    to_idx -= 1  # to_idx is exclusive
    dataframe = _create_trajectory_db_df()
    # In the case that there is no points in a trajectory or number of points
    # are less than threshold, return empty dataframe
    if (to_idx < from_idx) or ((to_idx - from_idx + 1)
                               <= POINTS_FOR_TRAJECTORY_THRESHOLD):
        return dataframe

    working_dataframe = trajectory_dataframe.truncate(
        before=from_idx, after=to_idx)
    ...
    column_subset = [
        ETA_COL,
        NAVIGATIONAL_STATUS_COL,
        DRAUGHT_COL,
        DESTINATION_COL]
    sorted_series_by_frequency = _find_most_recurring(
        working_dataframe, column_subset=column_subset, drop_na=False)
    ...
    # Ship information
    # Change 'Unknown' and 'Undefined' to NaN values, so they can be
    # disregarded
...
def _tfloat_from_dataframe(dataframe: gpd.GeoDataFrame,
                           float_column: str) -> TFloatInstSet:
    tfloat_lst = []
    for _, row in dataframe.iterrows():
        mobilitydb_timestamp = row[TIMESTAMP_COL].strftime(
            MOBILITYDB_TIMESTAMP_FORMAT) + '+01'
        float_val = str(row[float_column])
        tfloat_lst.append(TFloatInst(
            str(float_val + '@' + mobilitydb_timestamp)))

    return TFloatInstSet(*tfloat_lst)


def _find_most_recurring(dataframe: gpd.GeoDataFrame,
                         column_subset: List[str], drop_na: bool) -> pd.Series:
    return dataframe.value_counts(
        subset=column_subset, sort=True, dropna=drop_na).index.to_frame()


def _construct_stopped_trajectory(
        mmsi: int, trajectory_dataframe: gpd.GeoDataFrame, from_idx: int) -> pd.DataFrame:
    for idx in range(from_idx, len(trajectory_dataframe.index)):
        row = trajectory_dataframe.iloc[idx]

        if row[SOG_COL] >= STOPPED_KNOTS_THRESHOLD:
            stopped_trajectory = _finalize_trajectory(
                mmsi, trajectory_dataframe, from_idx, idx, infer_stopped=True)
            trajectories = _construct_moving_trajectory(
                mmsi, trajectory_dataframe, idx)
            return pd.concat([stopped_trajectory, trajectories])

    stopped_trajectory = _finalize_trajectory(
        mmsi, trajectory_dataframe, from_idx, len(
            trajectory_dataframe.index), infer_stopped=True)
    return stopped_trajectory


def _convert_dataframe_to_trajectory(
        trajectory_dataframe: pd.DataFrame) -> TGeomPointSeq:
    ...
    mobilitydb_dataframe[MBDB_TRAJECTORY_COL] = trajectory_dataframe[GEO_PANDAS_GEOMETRY_COL].astype(
        str) + '@' + mobilitydb_dataframe[TIMESTAMP_COL]
...
def _rebuild_to_geodataframe(
        pandas_dataframe: pd.DataFrame) -> gpd.GeoDataFrame:
    if GEO_PANDAS_GEOMETRY_COL in pandas_dataframe.columns:
        pandas_dataframe.drop(
            labels=GEO_PANDAS_GEOMETRY_COL,
            axis='columns',
            inplace=True)
    return gpd.GeoDataFrame(data=pandas_dataframe, geometry=gpd.points_from_xy(
        x=pandas_dataframe[LONGITUDE_COL], y=pandas_dataframe[LATITUDE_COL], crs=COORDINATE_REFERENCE_SYSTEM))
...
def _remove_outliers(dataframe: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    for idx in range(0, len(dataframe.index)):
        ...
        if not _check_outlier(cur_point=row, prev_point=prev_row,
                              speed_threshold=SPEED_THRESHOLD_KNOTS, dist_func=_euclidian_dist):
            prev_row = row
            result_dataframe = pd.concat([result_dataframe, row])

    return _rebuild_to_geodataframe(
        result_dataframe).to_crs(COORDINATE_REFERENCE_SYSTEM)


def _check_outlier(cur_point: gpd.GeoDataFrame, prev_point: gpd.GeoDataFrame,
    ...
    distance = dist_func(
        cur_point[GEO_PANDAS_GEOMETRY_COL].iloc[0].x,
        cur_point[GEO_PANDAS_GEOMETRY_COL].iloc[0].y,
        prev_point[GEO_PANDAS_GEOMETRY_COL].iloc[0].x,
        prev_point[GEO_PANDAS_GEOMETRY_COL].iloc[0].y)
    ...
    if abs((cur_point[SOG_COL].iloc[0] - speed)
           > COMPUTED_VS_SOG_KNOTS_THRESHOLD):
...
def _euclidian_dist(a_long: float, a_lat: float,
                    b_long: float, b_lat: float) -> float:
    return math.sqrt(
