...
from etl.constants import T_MMSI_COL, T_IMO_COL, T_SHIP_NAME_COL, T_SHIP_CALLSIGN_COL, T_A_COL, T_B_COL, T_C_COL, \
    T_D_COL, T_SHIP_TYPE_ID_COL, T_LOCATION_SYSTEM_TYPE_COL
...
class ShipDimensionInserter (BulkInserter):
    ...
    def ensure(self, df: pd.DataFrame, conn) -> pd.DataFrame:
        ...
        insert_query = """
            ...
            ship_type_id, location_system_type)
            VALUES {}
            RETURNING ship_id, mmsi, imo, callsign ship_callsign,
                name ship_name, a, b, c, d, ship_type_id, location_system_type
        """
        ...
        select_query = """
            SELECT
                ...
                ship_type_id, location_system_type
            FROM dim_ship
            WHERE (mmsi, imo, name, callsign, a, b, c, d, ship_type_id, location_system_type) IN {}
            """
