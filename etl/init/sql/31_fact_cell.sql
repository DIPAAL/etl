-- Create 5000m fact table
CREATE TABLE fact_cell_5000m (
    bounding_box stbox NOT NULL,

    sog float NOT NULL,
    delta_heading float,
    draught float,
    delta_cog float,

    cell_x integer NOT NULL,
    cell_y integer NOT NULL,
    ship_id integer NOT NULL,
    entry_date_id integer NOT NULL,
    entry_time_id integer NOT NULL,
    exit_date_id INTEGER NOT NULL,
    exit_time_id integer NOT NULL,
    trajectory_sub_id integer NOT NULL,
    division_id SMALLINT NOT NULL,
    direction_id smallint NOT NULL,
    nav_status_id smallint NOT NULL,
    infer_stopped boolean NOT NULL,
    -- Padding: 2+2+2+1 = 7 bytes, so 1 byte of padding to reach the MAXALIGN of 8 bytes

    PRIMARY KEY (cell_x, cell_y, ship_id, entry_date_id, entry_time_id, exit_date_id, exit_time_id, direction_id, nav_status_id, trajectory_sub_id, division_id),
    FOREIGN KEY (ship_id) REFERENCES dim_ship(ship_id),
    FOREIGN KEY (entry_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (entry_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (exit_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (exit_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (direction_id) REFERENCES dim_direction(direction_id),
    FOREIGN KEY (nav_status_id) REFERENCES dim_nav_status(nav_status_id),
    FOREIGN KEY (division_id) REFERENCES spatial_division(division_id)
) PARTITION BY RANGE (entry_date_id);

CREATE INDEX fact_cell_5000m_bounding_box_idx ON fact_cell_5000m USING spgist (bounding_box);

-- Create 1000m fact table
CREATE TABLE fact_cell_1000m (
    bounding_box stbox NOT NULL,

    sog float NOT NULL,
    delta_heading float,
    draught float,
    delta_cog float,

    cell_x integer NOT NULL,
    cell_y integer NOT NULL,
    ship_id integer NOT NULL,
    entry_date_id integer NOT NULL,
    entry_time_id integer NOT NULL,
    exit_date_id INTEGER NOT NULL,
    exit_time_id integer NOT NULL,
    trajectory_sub_id integer NOT NULL,
    division_id SMALLINT NOT NULL,
    direction_id smallint NOT NULL,
    nav_status_id smallint NOT NULL,
    infer_stopped boolean NOT NULL,
    -- Padding: 2+2+2+1 = 7 bytes, so 1 byte of padding to reach the MAXALIGN of 8 bytes

    PRIMARY KEY (cell_x, cell_y, ship_id, entry_date_id, entry_time_id, exit_date_id, exit_time_id, direction_id, nav_status_id, trajectory_sub_id, division_id),
    FOREIGN KEY (ship_id) REFERENCES dim_ship(ship_id),
    FOREIGN KEY (entry_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (entry_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (exit_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (exit_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (direction_id) REFERENCES dim_direction(direction_id),
    FOREIGN KEY (nav_status_id) REFERENCES dim_nav_status(nav_status_id),
    FOREIGN KEY (division_id) REFERENCES spatial_division(division_id)
) PARTITION BY RANGE (entry_date_id);

CREATE INDEX fact_cell_1000m_bounding_box_idx ON fact_cell_1000m USING spgist (bounding_box);

-- Create 200m fact table
CREATE TABLE fact_cell_200m (
    bounding_box stbox NOT NULL,

    sog float NOT NULL,
    delta_heading float,
    draught float,
    delta_cog float,

    cell_x integer NOT NULL,
    cell_y integer NOT NULL,
    ship_id integer NOT NULL,
    entry_date_id integer NOT NULL,
    entry_time_id integer NOT NULL,
    exit_date_id INTEGER NOT NULL,
    exit_time_id integer NOT NULL,
    trajectory_sub_id integer NOT NULL,
    division_id SMALLINT NOT NULL,
    direction_id smallint NOT NULL,
    nav_status_id smallint NOT NULL,
    infer_stopped boolean NOT NULL,
    -- Padding: 2+2+2+1 = 7 bytes, so 1 byte of padding to reach the MAXALIGN of 8 bytes

    PRIMARY KEY (cell_x, cell_y, ship_id, entry_date_id, entry_time_id, exit_date_id, exit_time_id, direction_id, nav_status_id, trajectory_sub_id, division_id),
    FOREIGN KEY (ship_id) REFERENCES dim_ship(ship_id),
    FOREIGN KEY (entry_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (entry_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (exit_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (exit_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (direction_id) REFERENCES dim_direction(direction_id),
    FOREIGN KEY (nav_status_id) REFERENCES dim_nav_status(nav_status_id),
    FOREIGN KEY (division_id) REFERENCES spatial_division(division_id)
) PARTITION BY RANGE (entry_date_id);

CREATE INDEX fact_cell_200m_bounding_box_idx ON fact_cell_200m USING spgist (bounding_box);

-- Create 50m fact table
CREATE TABLE fact_cell_50m (
    bounding_box stbox NOT NULL,

    sog float NOT NULL,
    delta_heading float,
    draught float,
    delta_cog float,

    cell_x integer NOT NULL,
    cell_y integer NOT NULL,
    ship_id integer NOT NULL,
    entry_date_id integer NOT NULL,
    entry_time_id integer NOT NULL,
    exit_date_id INTEGER NOT NULL,
    exit_time_id integer NOT NULL,
    trajectory_sub_id integer NOT NULL,
    division_id SMALLINT NOT NULL,
    direction_id smallint NOT NULL,
    nav_status_id smallint NOT NULL,
    infer_stopped boolean NOT NULL,
    -- Padding: 2+2+2+1 = 7 bytes, so 1 byte of padding to reach the MAXALIGN of 8 bytes

    PRIMARY KEY (cell_x, cell_y, ship_id, entry_date_id, entry_time_id, exit_date_id, exit_time_id, direction_id, nav_status_id, trajectory_sub_id, division_id),
    FOREIGN KEY (ship_id) REFERENCES dim_ship(ship_id),
    FOREIGN KEY (entry_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (entry_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (exit_date_id) REFERENCES dim_date(date_id),
    FOREIGN KEY (exit_time_id) REFERENCES dim_time(time_id),
    FOREIGN KEY (direction_id) REFERENCES dim_direction(direction_id),
    FOREIGN KEY (nav_status_id) REFERENCES dim_nav_status(nav_status_id),
    FOREIGN KEY (division_id) REFERENCES spatial_division(division_id)
) PARTITION BY RANGE (entry_date_id);

CREATE INDEX fact_cell_50m_bounding_box_idx ON fact_cell_50m USING spgist (bounding_box);