CREATE OR REPLACE PROCEDURE create_heatmap_50m (
    t_res INTEGER,
    s_res INTEGER,
    date_key INTEGER,
    part_id SMALLINT
...
BEGIN
    ...
    CREATE TEMPORARY TABLE heatmap_data AS
        SELECT
            ...
            i1.cell_x / (5000 / 50) AS cell_x,
            i1.cell_y / (5000 / 50) AS cell_y,
        ...
        (
            ...
            INNER JOIN dim_cell_50m dc ON dc.x = fc.cell_x AND dc.y = fc.cell_y AND dc.partition_id = fc.partition_id
        ...
        GROUP BY i1.cell_x / (5000 / 50), i1.cell_y / (5000 / 50), i1.hour_of_day, i1.ship_type_id;
