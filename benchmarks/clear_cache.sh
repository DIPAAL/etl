...
ssh -o "StrictHostKeyChecking=no" ubuntu@130.225.39.200 'sudo bash -c "sync; echo 3 > /proc/sys/vm/drop_caches" && microk8s kubectl rollout restart statefulset ais-citus-master ais-citus-worker-1 && microk8s kubectl rollout status statefulset ais-citus-master ais-citus-worker-1'
