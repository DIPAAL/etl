...
FROM (
    ...
    FROM reference, (
        SELECT ST_Union(q1.rast) AS rast FROM (
            ...
            FROM (
                SELECT
                    fch.partition_id, ST_Union(fch.rast, (SELECT union_type FROM dim_heatmap_type WHERE slug = 'delta_cog')) AS rast
                ...
                AND fch.heatmap_type_id = (SELECT heatmap_type_id FROM dim_heatmap_type WHERE slug = 'delta_cog')
