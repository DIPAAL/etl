...
def create_geopandas_dataframe(data_file_path: str) -> gpd.GeoDataFrame:
    ...
    pandas_df[TIMESTAMP_COL] = pandas_df[AIS_TIMESTAMP_COL].apply(
        func=lambda t: apply_datetime_if_not_none(t, CVS_TIMESTAMP_FORMAT))
    pandas_df[ETA_COL] = pandas_df[ETA_COL].apply(
...
@pytest.mark.parametrize('a_long, a_lat, b_long, b_lat, expected',
                         euclidean_testdata)
...
def to_minimal_outlier_detection_frame(
        long: float, lat: float, timestamp: str, sog: float):
    ...
    geo_test_frame = gpd.GeoDataFrame(
        data=test_frame,
...
test_data_is_outlier = [
    (to_minimal_outlier_detection_frame(56.8079,
                                        11.7168,
                                        '07/09/2021 00:00:00',
                                        2.5),
     to_minimal_outlier_detection_frame(55.8079,
                                        10.7168,
                                        '07/09/2021 00:00:00',
                                        2.5),
     100,
    ...
    (to_minimal_outlier_detection_frame(56.8079,
                                        ...
                                        '07/09/2021 00:00:00',
                                        2.5),
     to_minimal_outlier_detection_frame(57.8079,
                                        12.7168,
                                        '07/09/2021 00:06:02',
                                        2.5),
     1,
...
@pytest.mark.parametrize(
    'prev_point, curr_point, speed_threshold, distance_func, expected', test_data_is_outlier)
def test_check_outlier(prev_point, curr_point,
                       speed_threshold, distance_func, expected):
    assert _check_outlier(
        prev_point,
        curr_point,
        speed_threshold,
        distance_func) == expected
...
test_data_date_smart_key_extraction = [
    ...
    (datetime.strptime('31/01/2022 10:10:20', CVS_TIMESTAMP_FORMAT),
     20220131),  # Show that time does not matter
    (datetime.strptime('31/01/2022 13:14:15', CVS_TIMESTAMP_FORMAT),
     20220131)  # Show that time does not matter
]


@pytest.mark.parametrize('date, expected_smart_key',
                         test_data_date_smart_key_extraction)
...
test_data_time_smart_key_extraction = [
    ...
    # Show that date does not matter
    (datetime.strptime('01/01/2022 10:10:10', CVS_TIMESTAMP_FORMAT), 101010),
    # Show that date does not matter
    (datetime.strptime('31/01/2022 10:10:10', CVS_TIMESTAMP_FORMAT), 101010),
    # Show that date does not matter
    (datetime.strptime('24/12/2022 10:10:10', CVS_TIMESTAMP_FORMAT), 101010)
]


@pytest.mark.parametrize('time, expected_smart_key',
                         test_data_time_smart_key_extraction)
...
def test_find_most_recuring_drops_na():
    COL_1 = 'food'
    COL_2 = 'good'
    ...
    result = _find_most_recurring(
        dataframe=test_frame, column_subset=[
            COL_1, COL_2], drop_na=True)
    assert len(result.index) == expected_with_dropping_entries

    result = _find_most_recurring(
        dataframe=test_frame, column_subset=[
            COL_1, COL_2], drop_na=False)
...
def test_point_to_trajectory_threshold_under_returns_empty_frame():
    ...
    result_dataframe = _finalize_trajectory(
        mmsi=test_mmsi,
        trajectory_dataframe=None,
        from_idx=from_idx,
        to_idx=to_idx,
        infer_stopped=False)
...
def test_point_to_trajectory_threshold_on_returns_empty_frame():
    ...
    result_frame = _finalize_trajectory(
        mmsi=test_mmsi,
        trajectory_dataframe=None,
        from_idx=from_idx,
        to_idx=to_idx,
...
def test_point_to_trajectory_threshold_above_returns_trajectory():
    ...
    result_frame = _finalize_trajectory(
        mmsi=test_mmsi,
        trajectory_dataframe=test_dataframe,
        from_idx=from_idx,
        to_idx=to_idx,
        infer_stopped=False)
